...
class TestChatMessage:
    ...
    def test_query_by_role(self, db_session):
        ...
        user = User(email="test4@test.com", hashed_password="hash")
        db_session.add(user)
        db_session.commit()
        session = ChatSession(user_id=user.id)
        db_session.add(session)
    ...
    def test_created_at_auto_set(self, db_session):
        ...
        user = User(email="test5@test.com", hashed_password="hash")
        db_session.add(user)
        db_session.commit()
        session = ChatSession(user_id=user.id)
        db_session.add(session)
    ...
    def test_content_persists_long_text(self, db_session):
        ...
        user = User(email="test6@test.com", hashed_password="hash")
        db_session.add(user)
        db_session.commit()
        session = ChatSession(user_id=user.id)
        db_session.add(session)
